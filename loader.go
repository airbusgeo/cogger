...
// to out
//
// Deprecated: use Config.Rewrite in order to use non-default options
...
func (cfg Config) Rewrite(out io.Writer, readers ...tiff.ReadAtReadSeeker) error {
	...
	sort.Slice(ifds, func(i, j int) bool {
		...
		if ifds[i].ImageHeight != ifds[j].ImageHeight {
			return ifds[i].ImageHeight > ifds[j].ImageHeight
		}
		return ifds[i].SubfileType < ifds[j].SubfileType
	})
	if ifds[0].SubfileType != 0 {
		return fmt.Errorf("failed sort: first px=%d type=%d", ifds[0].ImageHeight, ifds[0].SubfileType)
	}
	curOvr := ifds[0]
